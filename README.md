...
Password-encrypted calls require a browser with `RTCRtpScriptTransform` support. Offers and answers include an HMAC confirmation of the derived media key, allowing both participants to reject the bridge with a clear error when their passwords differ without transmitting the password or key. The connection fails closed when encoded transforms are unavailable; it never sends audio or video protected only by WebRTC transport encryption. Calls start as audio-only. Use **Start video** during a call to grant camera access and share video, and **Stop video** to stop the camera and remove its WebRTC sender.
